...
SCHEMA_STATEMENTS = [
    ...
    CREATE TABLE IF NOT EXISTS faces (
        ...
        UNIQUE KEY uq_image_face (image_path(255), face_index)
    ) ENGINE=InnoDB;
    """,
    """
...
class Storage:
    ...
    def insert_face(self, identity: str, image_path: str, bbox: np.ndarray,
                     landmarks: np.ndarray, det_score: float,
                     face_index: int = 0, crop_path: str = None) -> int:
        cur = self.conn.cursor()
        cur.execute(
            """
            INSERT INTO faces (identity, image_path, face_index, bbox, landmarks, det_score, crop_path)
            VALUES (%s, %s, %s, %s, %s, %s, %s)
            ON DUPLICATE KEY UPDATE
                bbox = VALUES(bbox), landmarks = VALUES(landmarks),
                det_score = VALUES(det_score), crop_path = VALUES(crop_path)
            """,
            (identity, image_path, face_index,
             json.dumps(np.asarray(bbox).tolist()),
             json.dumps(np.asarray(landmarks).tolist()),
             float(det_score), crop_path),
        )
        self.conn.commit()
        if cur.lastrowid:
            ...
            cur.execute(
                "SELECT id FROM faces WHERE image_path = %s AND face_index = %s",
    ...
    def load_embeddings_df(self, model: str):
        """Returns a pandas DataFrame: face_id, identity, image_path, face_index, crop_path, vector."""
        import pandas as pd
        cur = self.conn.cursor(dictionary=True)
        cur.execute(
            """
            SELECT f.id AS face_id, f.identity, f.image_path,
                   f.face_index, f.crop_path, e.vector
