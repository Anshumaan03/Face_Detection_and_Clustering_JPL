...
def _get_detector():
    ...
    if _detector is None:
        ...
        _detector.prepare(ctx_id=config.DETECTOR_CTX_ID, det_size=config.DETECTION_SIZE)
        logger.info("InsightFace detector (%s) loaded with det_size=%s", 
                   config.INSIGHTFACE_DET_MODEL, config.DETECTION_SIZE)
    return _detector
...
def largest_or_best_face(faces: List[FaceDetection]) -> Optional[FaceDetection]:
    """Convenience: pick the single best-scoring face."""
    return faces[0] if faces else None


def filter_valid_faces(faces: List[FaceDetection], img_bgr: np.ndarray = None) -> List[FaceDetection]:
    """Strict quality filter to keep only good faces."""
    if not faces:
        return []
    valid = []
    for face in faces:
        ...
        if img_bgr is not None:
            try:
                ...
                if crop.size > 0:
                    ...
                    if blur_var < config.FACE_MIN_BLUR_VAR:
                        continue
            except:
                pass
        valid.append(face)
    valid.sort(key=lambda fd: fd.det_score, reverse=True)
    return valid
...
# ... (keep everything from your original common.py up to the end) ...