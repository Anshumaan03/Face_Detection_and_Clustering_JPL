...
with col1:
    model_choice = st.selectbox("Model", list(config.MODEL_INPUT_SPECS.keys()))

cluster_dir = os.path.join(config.CLUSTERS_ROOT, run_label, model_choice)
...
if not os.path.isdir(cluster_dir):
    ...
    cluster_folders = sorted(
        ...
        key=lambda x: (x != "noise", x)  # noise last-ish, otherwise alpha sort
    )
    with col2:
        cluster_choice = st.selectbox("Cluster", cluster_folders)

    chosen_dir = os.path.join(cluster_dir, cluster_choice)
    image_paths = sorted(glob.glob(os.path.join(chosen_dir, "*")))

    st.write(f"**{len(image_paths)} images** in `{model_choice}/{cluster_choice}`")

    # quick purity check: how many distinct identities ended up in this cluster?
